...
class Environment:
    def __init__(self, model, path, method, train_loader, test_loader, device):
        ...
        for i, m in enumerate(self.module_list):
            if type(m) in self.prunable_layer_types:
                if type(m) == nn.Conv2d and m.groups == m.in_channels:  # depth-wise conv, buffer
                    ...
                    buffer_dict[i] = this_buffer_list
        ...
        print('buffer layer is', self.buffer_layer)
        ...
        for idx in self.prunable_layer + self.buffer_layer:  # get all
            m = self.module_list[idx]
            m.old_forward = m.forward
            m.forward = new_forward(m)
        
        with torch.no_grad():
            input_data, _ = self.train_loader.__iter__().next()
            _ = self.model(input_data.float().to(self.device))
    ...
    def get_reward(self, final_state):
        with torch.no_grad():
            self.model.eval()
            x, y = self.train_loader.__iter__().next()
        ...
        FLOPs_ratio = final_state['Reduced'] / self.total_FLOPs
        
        reward = (batch_accuracy + FLOPs_ratio)/abs((batch_accuracy-FLOPs_ratio))
        
        return reward
    
    def validate(self, final_state):
        ...
        print("Pruned Model's FLOPs Ratio is %f" % FLOPs_ratio)
        
        reward = (accuracy + FLOPs_ratio)/abs((accuracy-FLOPs_ratio))
        print("="*100)
        print("Validation Reward is %f" % reward)
