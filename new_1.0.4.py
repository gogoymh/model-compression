...
parser.add_argument("--ver3", type=int, default=4, help="Version info")
parser.add_argument("--ver4", type=int, default=1, help="Version info")
...
parser.add_argument("--save", type=int, default=5, help="Number of saving")
...
class Main(object):
    ...
    def load_pretrain(self, pre_path=None, first=True):
        ...
        unstructured = 0
        for i in range(opt.layer):
            unstructured += cal_sparsity(self.parameter[i])
        ...
        for k in range(opt.ensemble//4):
            print("L_%02d: %f, L_%02d: %f, L_%02d: %f, L_%02d: %f" % 
                  ((4*k+1),L[4*k],(4*k+2),L[4*k+1],
                   (4*k+3),L[4*k+2],(4*k+4),L[4*k+3]))
        ...
        for k in range(opt.ensemble//4):
            print("S_%02d: %f, S_%02d: %f, S_%02d: %f, S_%02d: %f" %
                  ((4*k+1),S[4*k],(4*k+2),S[4*k+1],
                   (4*k+3),S[4*k+2],(4*k+4),S[4*k+3]))
        
        self.prob = make_prob(opt.alpha*S,L,opt.ensemble)  
    ...
    def fine_tuning(self, epoch, fine):
        ...
        for step in range(fine):            
            for j in range(opt.ensemble):
                ...
                ft1_start = timeit.default_timer()
                for i in range(opt.layer):
                    self.params[j][i] *= self.masks[j][i]
                    self.parameter[i].data = self.params[j][i].clone().to(self.device)
                ft1_stop = timeit.default_timer()
                print("Fine tuning time 1 is",(ft1_stop-ft1_start))
                
                ft2_start = timeit.default_timer()
                ...
                ft2_stop = timeit.default_timer()
                print("Fine tuning time 2 is",(ft2_stop-ft2_start))
                
                ft3_start = timeit.default_timer()
                ...
                if cal_sparsity(self.parameter[0].grad).item() == self.parameter[0].data.reshape(-1,).shape[0]:
                    ...
                    print("!" * 30)
                ft3_stop = timeit.default_timer()
                print("Fine tuning time 3 is",(ft3_stop-ft3_start))
                
                ft4_start = timeit.default_timer()
                #### save weight ####
                for i in range(opt.layer):
                    self.params[j][i] = self.parameter[i].data.clone()
                ft4_stop = timeit.default_timer()
                print("Fine tuning time 4 is",(ft4_stop-ft4_start))
        ...
        for j in range(opt.ensemble):
            ...
            unstructured = 0
            for i in range(opt.layer):
                unstructured += cal_sparsity(self.params[j][i].data)
        ...
        for k in range(opt.ensemble//4):
            print("Loss_%02d: %f, Loss_%02d: %f, Loss_%02d: %f, Loss_%02d: %f" % 
                  ((4*k+1),self.last_losses[4*k],(4*k+2),self.last_losses[4*k+1],
                   (4*k+3),self.last_losses[4*k+2],(4*k+4),self.last_losses[4*k+3]))        
        ...
        S = minmax_fn(S)
        for j in range(16):
            self.total_sparsity[:, epoch % opt.save] = S
        ...
        print("Best model is %d" % (self.best_model_tmp+1))
