                    ...
                    if cal_sparsity(self.params[j][0].grad).item() == self.params[0][0].data.reshape(-1,).shape[0]:
                        ...
                        for i in range(2,9,2):
                            if cal_sparsity(self.params[j][i].grad).item() == self.params[0][i].data.reshape(-1,).shape[0] :
                                collapsed += 2
                        if collapsed >= 2:
                            self.params[j][collapsed-2].data += opt.recovery * (self.params[j][collapsed-2].data == 0).float() * (torch.FloatTensor(self.params[j][collapsed-2].data.shape).uniform_() > opt.remask).float().cuda()
                        self.params[j][collapsed].data += opt.recovery * (self.params[j][collapsed].data == 0).float() * (torch.FloatTensor(self.params[j][collapsed].data.shape).uniform_() > opt.remask).float().cuda()
                        print("!" * 30)
            ...
            if epoch >= opt.buffer:
                ...
                for i in range(opt.layer):
                    ...
                    for j in range(opt.ensemble):
                        ...
                        self.masks[j][i] = copies[j].clone().detach().float().cuda()
