...
#### initial mask ####
torch.manual_seed(1)
mask_01 = torch.FloatTensor(params_01[0].data.shape).uniform_() > 0.5
torch.manual_seed(2)
mask_02 = torch.FloatTensor(params_02[0].data.shape).uniform_() > 0.5
torch.manual_seed(3)
mask_03 = torch.FloatTensor(params_03[0].data.shape).uniform_() > 0.5
torch.manual_seed(4)
mask_04 = torch.FloatTensor(params_04[0].data.shape).uniform_() > 0.5
...
def cross4D_over(mask1, mask2): # 4차원 교차 함수
  ...
  dim = mask1.dim()
  limit = mask1.shape[dim-4] if mask1.shape[dim-4] != 1 else 2
...
def cross3D_over(mask1, mask2): # 3차원 교차 함수
  ...
  dim = mask1.dim()
  limit2 = mask1.shape[dim-3] if mask1.shape[dim-3] != 1 else 2
...
def cross2D_over(mask1, mask2): # 2차원 교차 함수
  ...
  dim = mask1.dim()
  limit3 = mask1.shape[dim-2] if mask1.shape[dim-2] != 1 else 2
...
def cross1D_over(mask1, mask2): # 1차원 교차 함수
  ...
  dim = mask1.dim()
  limit4 = mask1.shape[dim-1] if mask1.shape[dim-1] != 1 else 2
...
###################################### 실험 1.0.0 #### 모델 4개, 레이어 conv1 mutate prob 0.01#######################
###################################### different initialize, not sharing parameters #################################

n_epochs = 10000 # 학습 횟수
...
for epoch in range(n_epochs):
  ...
  #### weight removing ####
  params_01[0].data = params_01[0].data * mask_01 
  params_02[0].data = params_02[0].data * mask_02 
  params_03[0].data = params_03[0].data * mask_03 
  params_04[0].data = params_04[0].data * mask_04 
  ...
  for batch_idx, (x, y) in enumerate(train_loader):
    ...
    loss_01.backward()
    params_01[0].grad = params_01[0].grad * mask_01 # gradient removing
    optimizer_01.step()
    ...
    loss_02.backward()
    params_02[0].grad = params_02[0].grad * mask_02 # gradient removing
    optimizer_02.step()
    ...
    loss_03.backward()
    params_03[0].grad = params_03[0].grad * mask_03 # gradient removing
    optimizer_03.step()
    ...
    loss_04.backward()
    params_04[0].grad = params_04[0].grad * mask_04 # gradient removing
    optimizer_04.step()
  ...
  ## mutation ##
  simple_mutate(copy[0],0.1)
  simple_mutate(copy[1],0.1)
  simple_mutate(copy[2],0.1)
  simple_mutate(copy[3],0.1)
