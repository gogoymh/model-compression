...
class DDPG_Agent:
    ...
    def searching_action4(self, episode, current_state, current_state_idx, data_controller):
        if self.memory.n_entries < self.warmup:
            if ((episode+1) % 2) == 0:
                action = self.random_heuristic_action()
    ...
    def random_heuristic_action(self):
        action = np.random.uniform(0,1,self.num_actions)
        if self.num_actions == 3:
            dice = np.random.choice(3,1)[0]
            if dice == 0:
                action[1:] = 1
            elif dice == 1:
                action[0] = 1
