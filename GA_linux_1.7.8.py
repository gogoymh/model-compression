...
parser.add_argument("--ver3", type=int, default=8, help="Version info")
    ...
    def train(self):
        ...
        for epoch in range(opt.n_epochs):
            ...
            if epoch >= opt.buffer:
                ...
                for j in range(opt.ensemble):
                    ...
                    if cal_sparsity(self.params[j][0].grad).item() == self.params[0][0].data.reshape(-1,).shape[0]:
                        print("!" * 30)
                        print("Model %d will be replaced with %d" %(j, (self.argmax_prob+1)))
                        for i in range(opt.layer):
            ...
            for batch_idx, (x, y) in enumerate(self.train_loader):
                ...
                self.total_loss[0,epoch] += loss0.item()
                self.total_loss[1,epoch] += loss1.item()
                self.total_loss[2,epoch] += loss2.item()
                self.total_loss[3,epoch] += loss3.item()
                self.total_loss[4,epoch] += loss4.item()
                self.total_loss[5,epoch] += loss5.item()
                self.total_loss[6,epoch] += loss6.item()
                self.total_loss[7,epoch] += loss7.item()
                self.total_loss[8,epoch] += loss8.item()
                self.total_loss[9,epoch] += loss9.item()
                self.total_loss[10,epoch] += loss10.item()
                self.total_loss[11,epoch] += loss11.item()
                self.total_loss[12,epoch] += loss12.item()
                self.total_loss[13,epoch] += loss13.item()
                self.total_loss[14,epoch] += loss14.item()
                self.total_loss[15,epoch] += loss15.item()
            ...
            if epoch >= opt.buffer:
                #print(self.total_sparsity[:,epoch].reshape(opt.ensemble))
