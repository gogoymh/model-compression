...
print("The Layer has %d parameters" % params_01[4].data.reshape(-1,).shape[0])


#### initial mask ####
torch.manual_seed(1)
mask_01 = torch.FloatTensor(params_01[4].data.shape).uniform_() > 0.9
torch.manual_seed(2)
mask_02 = torch.FloatTensor(params_02[4].data.shape).uniform_() > 0.9
torch.manual_seed(3)
mask_03 = torch.FloatTensor(params_03[4].data.shape).uniform_() > 0.9
torch.manual_seed(4)
mask_04 = torch.FloatTensor(params_04[4].data.shape).uniform_() > 0.9
torch.manual_seed(5)
mask_05 = torch.FloatTensor(params_05[4].data.shape).uniform_() > 0.9
torch.manual_seed(6)
mask_06 = torch.FloatTensor(params_06[4].data.shape).uniform_() > 0.9
torch.manual_seed(7)
mask_07 = torch.FloatTensor(params_07[4].data.shape).uniform_() > 0.9
torch.manual_seed(8)
mask_08 = torch.FloatTensor(params_08[4].data.shape).uniform_() > 0.9
...
def make_prob(minmax_Sparse, minmax_Loss): # minmax된 sparsity와 loss로 object function을 만들고 확률화 하는 함수
  ...
  result = total/total.sum() if total.sum() != 0 else torch.ones((8))*0.125
  return result
...
n_epochs = 2000 # 학습 횟수
print("Total epoch is %d" % n_epochs)
alpha = 1
...
total_sparsity_01 = torch.empty((n_epochs))
total_sparsity_02 = torch.empty((n_epochs))
total_sparsity_03 = torch.empty((n_epochs))
total_sparsity_04 = torch.empty((n_epochs))
total_sparsity_05 = torch.empty((n_epochs))
total_sparsity_06 = torch.empty((n_epochs))
total_sparsity_07 = torch.empty((n_epochs))
total_sparsity_08 = torch.empty((n_epochs))
...
for epoch in range(n_epochs):
  ...
  #### weight removing ####
  params_01[4].data *= mask_01 
  params_02[4].data *= mask_02 
  params_03[4].data *= mask_03 
  params_04[4].data *= mask_04 
  params_05[4].data *= mask_05
  params_06[4].data *= mask_06 
  params_07[4].data *= mask_07 
  params_08[4].data *= mask_08 
  ...
  for batch_idx, (x, y) in enumerate(train_loader):
    ...
    loss_01.backward()
    params_01[4].grad *= mask_01 # gradient removing
    optimizer_01.step()
    ...
    loss_02.backward()
    params_02[4].grad *= mask_02 # gradient removing
    optimizer_02.step()
    ...
    loss_03.backward()
    params_03[4].grad *= mask_03 # gradient removing
    optimizer_03.step()
    ...
    loss_04.backward()
    params_04[4].grad *= mask_04 # gradient removing
    optimizer_04.step()
    ...
    loss_05.backward()
    params_05[4].grad *= mask_05 # gradient removing
    optimizer_05.step()
    ...
    loss_06.backward()
    params_06[4].grad *= mask_06 # gradient removing
    optimizer_06.step()
    ...
    loss_07.backward()
    params_07[4].grad *= mask_07 # gradient removing
    optimizer_07.step()
    ...
    loss_08.backward()
    params_08[4].grad *= mask_08 # gradient removing
    optimizer_08.step()
  ...
  #### record ####
  total_sparsity_01[epoch] = cal_sparsity(params_01[4].data)
  total_sparsity_02[epoch] = cal_sparsity(params_02[4].data)
  total_sparsity_03[epoch] = cal_sparsity(params_03[4].data)
  total_sparsity_04[epoch] = cal_sparsity(params_04[4].data)
  total_sparsity_05[epoch] = cal_sparsity(params_05[4].data)
  total_sparsity_06[epoch] = cal_sparsity(params_06[4].data)
  total_sparsity_07[epoch] = cal_sparsity(params_07[4].data)
  total_sparsity_08[epoch] = cal_sparsity(params_08[4].data)
...
plt.axis([0,n_epochs,0,params_01[4].data.reshape(-1,).shape[0]])
plt.show()

print('Rememver to save best one in 8 models')

'''
torch.save({'epoch': n_epochs,
            'model_state_dict': model_01.state_dict(),
            'optimizer_state_dict': optimizer_01.state_dict(),
            'loss': total_loss_01}, 'C:/유민형/개인 연구/model compression/models/best_1.3.5.pkl')
'''
