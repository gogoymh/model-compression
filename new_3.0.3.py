...
parser.add_argument("--ver1", type=int, default=3, help="Version info")
parser.add_argument("--ver2", type=int, default=0, help="Version info")
parser.add_argument("--ver3", type=int, default=3, help="Version info")
parser.add_argument("--ver4", type=int, default=2, help="Version info")
...
parser.add_argument("--save", type=int, default=3, help="Number of saving")
...
class Main(object):
    ...
    def load_data(self):
        ...
        self.test_loader = DataLoader(
                ...
                batch_size=100, shuffle=False, pin_memory=True, num_workers = 5)
    ...
    def load_pretrain(self, pre_path=None, first=True):
        ...
        self.parameter_original = []
        for i in range(opt.layer):
            self.parameter_original.append(self.parameter[i].data.clone().detach()) 
    ...
    def evaluate(self):
        ...
        for j in range(opt.ensemble):
            for i in range(opt.layer):
                self.parameter[i].data = self.parameter_original[i].clone().detach()
                self.parameter[i].data *= self.masks[j][i]
            ...
            unstructured = 0
            for i in range(opt.layer):
                unstructured += cal_sparsity(self.parameter[i].data)
        
            channel_bonus, parallel_bonus, rectangular_bonus = cal_ch(self.parameter, self.changable, opt.layer)
        ...
        for k in range(4):
            self.last_sparsities[k,:] = minmax_fn(self.last_sparsities[k,:])
        ...
        #### minmax regularize ####
        A = minmax_fn(self.last_accuracies)
        S = minmax_fn(S)
        
        self.prob = make_prob(opt.alpha*S,A,opt.ensemble)  
        ...
        print("The best model is %d" % (self.best_model_tmp+1))
    ...
    def fine_tuning(self, epoch, fine):
        ...
        self.model.train()
        j = self.best_model_tmp
        for step in range(fine):
            for i in range(opt.layer):
                self.parameter[i].data = self.parameter_original[i]
                self.parameter[i].data *= self.masks[j][i]
        ...
        with torch.no_grad():
            ...
            print("Fine-tuned accuracy: %f" % accuracy)
    ...
    def save_result(self, epoch):
        ...
        for i in range(opt.layer):
                self.parameter[i].data = self.parameter_original[i]
                self.parameter[i].data *= self.masks[self.best_model_tmp][i]
...
if __name__ == '__main__':
    ...
    obj.load_pretrain("/home/super/ymh/modelcomp/results/Resnet18_Cifar10_2.pkl", first=True)
    obj.evaluate()
    ...
    for epoch in range(opt.n_epochs):
        ...
        print("^"*50)
        print("<"*50)
        print("남은 시간: %d시간 %d분" % (((total_stop-total_start)*(opt.n_epochs-epoch-1)) // 3600, (((total_stop-total_start)*(opt.n_epochs-epoch-1)) % 3600) / 60))
        print(">"*50)
        print("^"*50)
