...
print("Mutation prob is                | %f" % opt.mutate)
...
##########################################################################################################################
params = [params_01,params_02,params_03,params_04,params_05,params_06,params_07,params_08,
          params_09,params_10,params_11,params_12,params_13,params_14,params_15,params_16]
...
for epoch in range(opt.n_epochs):
  ...
  if epoch >= opt.buffer:
      ...
      for j in range(opt.ensemble):
          if cal_sparsity(params[j][0].grad).item() == 150 :
              for i in range(0,9,2):
                  params[j][i].data += opt.recovery * (params[j][i].data == 0).float() * (torch.FloatTensor(params[j][i].data.shape).uniform_() > 0.5).float().cuda()
              
              print("!" * 30)
              print("Model %d was broken. It's recovered!" % (j+1))
              print("!" * 30)
