...
class Environment:
            ...
            if type(m) in self.prunable_layer_types:
                if type(m) == nn.Conv2d and m.groups == m.in_channels:  # depth-wise conv, buffer
                    if m.groups == m.in_channels:
                        print(i,"m.groups == m.in_channels")
                    pass
                else:
    ...
    def cal_zero_action_loss(self):
        ...
        _, loss, _ = self.get_reward(state)
        #loss = -loss
        print(loss)
        self.max_loss = loss
    ...
    def test(self, final_state):
        ...
        reward = min(loss, self.max_loss) * math.log(max(int_FLOP_base, FLOPs))
    ...
    def original_result(self):
        ...
        print("Original Model's Reward is %f" % loss)
