...
def simple_mutate(mask, prob): # 돌연변이 함수 customized for conv3 layer
  loc = torch.LongTensor(4).random_(0, 5)
  loc[0] = torch.LongTensor(1).random_(0, mask.shape[0])
  loc[1] = torch.LongTensor(1).random_(0, mask.shape[1])
