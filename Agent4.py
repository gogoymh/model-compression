...
class DDPG_Agent:
    ...
    def update_network(self, current_states, actions, next_states, rewards, terminals): # input type is numpy array
        ...
        for i in range(self.num_steps):
            ...
            ## ---- Update memory ---- ##
            self.update_memory(current_states[i], actions[i], next_states[i], rewards[i], terminals[i], td_error, 1/Q_value) # store numpy in cpu
        ...
        if self.memory.n_entries <= self.warmup:
            ...
            actor_reward = np.vstack(critic_batch[3])
