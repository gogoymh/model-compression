...
parser.add_argument("--remask", type=float, default=0.7, help="Recovery masking rate")
      ...
      for j in range(opt.ensemble):
          if cal_sparsity(params[j][0].grad).item() == 150 :
              for i in range(0,9,2):
                  params[j][i].data += opt.recovery * (params[j][i].data == 0).float() * (torch.FloatTensor(params[j][i].data.shape).uniform_() > opt.remask).float().cuda()
