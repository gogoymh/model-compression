...
class DDPG_Agent:
    ...
    def searching_action(self, episode, current_state, current_state_idx):
    ...
    def update_network(self, current_states, actions, next_states, rewards, terminals): # input type is numpy array
        ...
        for i in range(self.num_steps):
            ...
            critic_priority = (abs(td_error.item()) + self.epsilon) ** self.alpha # proportional variant
            ...
            actor_priority = (abs(Q_value.item()) + self.epsilon) ** self.alpha
            
            ## ---- Update memory ---- ##
            self.update_memory(current_states[i], actions[i], next_states[i], rewards[i], terminals[i], critic_priority, actor_priority) # store numpy in cpu
        ...
        if self.memory.n_entries <= self.warmup:
            ...
            for i in range(self.batch_size):
                ## ---- get sample ---- ##
                critic_batch, critic_idx, critic_weight = self.memory.priority_sample(1, True)
                critic_transition = critic_batch[0]
                
                critic_current_state = torch.from_numpy(critic_transition[0]).float().to(self.device)
                critic_action = torch.from_numpy(critic_transition[1]).float().to(self.device)
                critic_next_state = torch.from_numpy(critic_transition[2]).float().to(self.device)
                critic_reward = torch.from_numpy(critic_transition[3]).float().to(self.device)
                critic_terminal = torch.from_numpy(critic_transition[4]).float().to(self.device)
                ...
                new_critic_priority = (abs(td_error.item()) + self.epsilon) ** self.alpha # proportional variant
                
                self.memory.update_priority(critic_idx[0], new_critic_priority, True)
                
                value_loss += torch.from_numpy(critic_weight).float().to(self.device) * (td_error ** 2)
            ...
            for i in range(self.batch_size):
                ## ---- get sample ---- ##
                actor_batch, actor_idx, actor_weight = self.memory.priority_sample(1, False)
                actor_transition = actor_batch[0]
                
                actor_current_state = torch.from_numpy(actor_transition[0]).float().to(self.device)
                ...
                new_actor_priority = (abs(Q_value.item()) + self.epsilon) ** self.alpha
                
                self.memory.update_priority(actor_idx[0], new_actor_priority, False)
                
                policy_loss -= torch.from_numpy(actor_weight).float().to(self.device) * Q_value
...
class Prioritized_Experience_Replay:
    ...
    def add(self, current_state, action, next_state, reward, terminal, critic_priority, actor_priority):
        ...
        self.critic_priorities[self.buffer_idx] = critic_priority
        self.actor_priorities[self.buffer_idx] = actor_priority
